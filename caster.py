...
"""Autocaster: Given a model of what roles in your game have which features, and
a csv of app information (say, collected from Google Forms), find and show an 
optimal matching of apps to roles.
...
Then you'll need to adjust runs and cols, add a model of your
characters, and collect your apps.  If you have problems, please call!
...
bias = 3
"bias: the bonus to score for casting someone at all, even if their app says they're indifferent"

# We only have enough apps for one, so comment out the others to save runtime
...
#
# Check run assignments, rejected roles, and the requirement that some roles be
# experienced.
def permit(app,char):
    timing = False
    for day in app['days']:
        if day in characters[char]:
            for attr in characters[char]:
                if attr in app and app[attr]==-1:
                    return False
            return not ("experienced" in characters[char] and app["first"]=="Yes")
    return False
...
def setup_z3(rawApps):
    ...
    s.maximize(z3scores(apps,roles,fullApps))
